...
### iter-174 (2026-06-17) — the backchannel driver: composing the clock + monitor (#7)

- **Shipped:** `session/backchannel_driver.py` — `BackchannelDriver`, the one
  object that owns a `MonologueClock` and a `BackchannelMonitor` and exposes
  `on_speech_start(now)` / `on_speech_stop(now)` / `observe(now)`. iter-173
  declared the live wiring "a thin adapter," but that adapter was **two objects
  + a four-line composition ritual** repeated by hand (both integration tests in
  `test_monologue_clock.py` spell it out) — and the `monitor.observe` line
  carried a **latent crash**.
- **Bug this surfaced and fixed:** the bare composition passes
  `clock.monologue_start_at` straight into `BackchannelMonitor.observe`, but
  that property is `None` until the first speech start — and `observe` computes
  `now - monologue_start_at` with **no None guard**, so an `observe` tick that
  lands *before any speech* (a warm-up poll, a timer on a freshly-`reset`
  session) raises `TypeError: unsupported operand type(s) for -: 'float' and
  'NoneType'` (confirmed by a standalone repro this lap). `BackchannelDriver.observe`
  short-circuits a no-monologue tick to a no-emit `BackchannelDecision`
  (`user_speaking_secs=0.0`, `cue_type=None`, the real `secs_since_last_backchannel`
  if it ever emitted) — the correct organic answer (no speaker to backchannel)
  *and* the crash guard — **without touching monitor state**, so the rate limit
  and the half-duplex invariant survive intact.
- **Half-duplex inherited verbatim:** every method beyond the None-guard is pure
  delegation, so a default (half-duplex) `FullDuplexConfig` ⇒ `observe` returns
  `emit=False` for every tick and the monitor never mutates — byte-for-byte the
  proven path. `reset()` clears the clock fully and the monitor's rate-limit
  clock, but (like the monitor's own `reset`) keeps the lifetime `emit_count`
  and the cue-rotation position, so a fresh monologue continues the rotation.
- **19 unit tests** (`tests/unit/test_backchannel_driver.py`, file-path-loaded
  like its siblings): construction / config forwarding / injected collaborators;
  the None-tick crash guard (before speech, after reset, secs-since reporting,
  pause echo); end-to-end emit in the clause-pause window; monologue continuity
  across a clause pause; cue rotation across consecutive emits; rate-limit HOLD;
  half-duplex never-emit + never-mutate; passthrough views; reset semantics.

**Next:** backlog #7's pure stack is now *complete and composed* — the live
`pipecat_server.py` `Broadcaster` wiring is a truly thin shim (`on_speech_*`
off the VAD frames, `observe(now)` on a tick, `broadcast_cue(d.cue_type)` on
emit), still blocked only on the absent pipecat dep on the x86_64 runner. The
sibling `should_abandon_turn` (iter-152) → `mic_chat` barge-path wiring also
remains, blocked on no transcript at VAD-trigger time.

### iter-175 (2026-06-17) — agent-backchannel emission count in the session summary (#8)

**Branch:** `iter-175-backchannel-emit-metric` (merged ff to main)

- The #7 pure stack (iter-153/170/171/172/173/174) is complete and composed,
  and the live `Broadcaster` wiring is pipecat-blocked on the x86_64 runner.
  But the #8 *measurement surface* — the "Organic turn-taking" summary block
  (`_emit_organic_block`) — had **no presence for the agent's own backchannels**
  even though `BackchannelDriver` / `BackchannelMonitor` already track the
  lifetime `emit_count`. The block measured false-endpoints, the *user's*
  continuers held (iter-154), held/displaced utterances (iter-161), and merge
  caps (iter-163), but the *agent's* mid-speech "mhmm"s — the whole point of #7
  — were invisible. So #7's emission would be *asserted, not measured* the
  moment the live path lights up.
- **Shipped:** `OrganicStats.backchannels_emitted` + a "Backchannels" line in
  `_emit_organic_block` (`examples/_chat_metrics.py`) — the agent-side mirror of
  the user-side "Continuers held" line. `SessionMeta.backchannels_emitted`
  (SessionMeta-only, no legacy kwarg path) carries the driver's `emit_count`
  into the summary; `print_session_summary` threads it to both the main
  `_emit_organic_block` call and the zero-completed-turns early return (a session
  can backchannel a long user monologue then quit before any turn completes).
  The whole block stays fully suppressed when every counter is zero — the
  half-duplex / no-driver default — so today's summaries are byte-for-byte
  unchanged.
- **+8 unit tests** (`tests/unit/test_emit_organic_block.py`): the
  `OrganicStats` default; emitted-alone emits the block + line; zero omits the
  line; emitted-alone doesn't emit the other lines; all signals under one
  header; default-meta omits; meta surfaces it; surfaces on the zero-turn early
  return; alongside continuers both surface under one header.
- **Verification:** `test_emit_organic_block.py` → 43 passed (35 + 8). Full unit
  suite 2382 passed (2374 + 8). Integration 30 passed, 1 skipped.
  `py_compile examples/_chat_metrics.py` clean.

**Next:** the live `pipecat_server.py` `Broadcaster` wiring for #7 (feed the
VAD frames to a `BackchannelDriver`, `broadcast_cue` on emit, and surface its
`emit_count` into `SessionMeta.backchannels_emitted` — the metric this lap
...
### iter-177 (2026-06-17) — `BackchannelMonitor.secs_since_last_backchannel(now)`: the monitor-side accessor mirror (#7)

- **Shipped:** `BackchannelMonitor.secs_since_last_backchannel(now)` — the
  monitor-side mirror of iter-176's `MonologueClock.user_speaking_secs(now)`.
  `decide_backchannel_timing` consumes three derived quantities; iter-176 folded
  *two* of them (`user_speaking_secs`, `pause_secs`) onto the clock so the
  driver's no-monologue short-circuit stopped hand-recomputing them. But the
  **third** — `secs_since_last_backchannel` (`now - last_emit`, `None` if never)
  — is the one quantity a clock *can't* answer: it depends on the monitor's own
  past EMIT decisions, so the monitor is its owner. Yet the driver's no-monologue
  path still hand-recomputed it (`None if last_emit is None else max(0.0, now -
  last_emit)`), **duplicating the exact None-guard + skew-clamp derivation inside
  `BackchannelMonitor.observe`**. This lap folds that derivation onto one monitor
  method so both the monitor's own `observe` and the driver's short-circuit
  source it from the single owner of `_last_backchannel_at` and can't drift.
- **Both call sites rewired:** `BackchannelMonitor.observe` now derives
  `secs_since = self.secs_since_last_backchannel(now)` instead of the inline
  `if/else` (behavior-identical); `BackchannelDriver.observe`'s no-monologue
  short-circuit drops its local `last_emit`/`max(0.0, …)` recompute and calls
  `self._monitor.secs_since_last_backchannel(now)`. Pure delegation, no behavior
  change — the existing iter-174 None-tick tests (`reports`, `is_none_when_never`)
  still pass byte-for-byte.
- **+6 unit tests:** 5 in `test_backchannel_monitor.py` (None before first emit;
  measures from last emit; clamps against skew; None after `reset`;
  observe-matches-accessor consistency) + 1 in `test_backchannel_driver.py`
  (the no-monologue short-circuit reports exactly the monitor accessor's value).
- **Verification:** `test_backchannel_monitor.py` + `test_backchannel_driver.py`
  → 46 passed (40 + 6). Full unit suite 2395 passed (2389 + 6). Integration 30
  passed, 1 skipped. `py_compile session/backchannel_monitor.py
  session/backchannel_driver.py` clean.
...
### iter-180 (2026-06-17) — the cue-window partition invariant (#7 hardening)

- **The gap (the sibling of iter-179, the inequality this time).** iter-179
  pinned the three scalars that are deliberately *equal* (`reset_gap_secs ==
  max_pause_secs == silence_floor_secs` = 2.0). But the organic stack also has
  two **distinct backchannel-cue paths** designed to own *disjoint* stretches of
  the trailing-silence axis: the **mid-speech backchannel** path
  (`backchannel_timing.py`) emits "mhmm" during a brief clause pause, window
  `[min_pause, max_pause)` = `[0.3, 2.0)`; the **turn-end cue** path
  (`turn_taking.py`'s `TurnTakingEngine`) fires a silence-driven `PLAY_CUE` once
  trailing silence reaches `silence_backchannel_min` = 4.0s. The
  `backchannel_timing.py` docstring states the contract — "the two paths
  partition the silence axis cleanly" — but **nothing tested the no-overlap
  inequality** `max_pause_secs <= silence_backchannel_min`.
  ...
  every per-module test would stay green because none looks at the sibling.
- **Shipped:** `tests/unit/test_cue_window_partition_invariant.py` — 4 tests
  derived from both modules themselves (loaded by file path), not from literals:
  the headline `max_pause_secs <= silence_backchannel_min`; a disjointness
  spot-check at the `max_pause` boundary (excluded from the half-open mid-speech
  window, still below the turn-end open); a well-formed-window sanity
  (`min_pause < max_pause` so "partition" is meaningful); and one recording the
  present edges (2.0s / 4.0s, with the 2.0s pure-silence gap between) so an
  intentional retune is a deliberate edit.
- **Doc notes** added to `turn_taking.py`'s `silence_backchannel_min` and
  `backchannel_timing.py`'s module + `max_pause_secs` docstrings, and the README
  Research section, each naming the iter-180 guard.
- **Verification:** `test_cue_window_partition_invariant.py` → 4 passed.
  Confirmed it catches drift: setting `silence_backchannel_min = 1.5` turns 3 of
  4 red (the well-formed-window test correctly stays green — the mid-speech
  window itself didn't change). Full unit suite 2406 passed (2402 + 4).
  Integration 30 passed, 1 skipped. `py_compile` clean on both touched modules +
  the new test.

**Next:** unchanged from iter-179 — the live `pipecat_server.py` `Broadcaster`
wiring for #7 (feed VAD frames to a `BackchannelDriver`, `broadcast_cue` on
emit, surface `emit_count`) is the headline follow-on, blocked on the absent
pipecat dep; the `should_abandon_turn` (iter-152) → `mic_chat` barge-path wiring
remains blocked on no transcript at VAD-trigger time.
