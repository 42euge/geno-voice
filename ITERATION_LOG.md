...
## iter-194 — extend the client JS test suite to the full ContinuousListener state machine

**Branch:** `iter-194-js-state-tests` (merged ff to main, commit `3801825`)
**Date:** 2026-06-17

**Improvement (iter-193 next-planned #4):** iter-193 introduced the repo's first
browser-client unit suite (`client/voice-capture.test.js`, 11 tests) but scoped
it to the pre-roll ring buffer. The rest of the `ContinuousListener` VAD state
machine — silence timeout → `onSpeechEnd`, the `minSpeechMs` / empty-chunks
drops, the mute/unmute gate, the raw-frame callback, the active/inactive guard,
and the `onStateChange` transition sequence — had no JS coverage. This lap fills
that gap so future client edits (the queued `gainNode` knob, the 200ms→100ms
debounce change) have a regression net.

What changed:
- **`client/voice-capture.test.js`** — 9 new tests (11 → 20 total). New
  `withClock(body)` harness keeps a controllable `performance.now()` live across
  a whole scenario so a test can commit speech, advance the clock past
  `silenceDurationMs`, then invoke `_onSilence()` directly (the same method the
  real `setTimeout` would call) — deterministic, no wall-clock waiting. A
  `commitSpeech` helper holds loud frames past the 200ms debounce. Coverage:
  - **silence → onSpeechEnd**: committed segment encodes a real RIFF WAV
    (`>44` bytes), `chunks` cleared, state walks `speaking → processing`.
  - **minSpeechMs drop**: an utterance shorter than `minSpeechMs` fires no
    `onSpeechEnd`; state returns `speaking → listening`.
  - **_onSilence no-op** when not speaking (chunks untouched).
  - **mute gate**: loud frames while muted meter `frameCount`/`lastRms` but
    never form a candidate or change state.
  - **unmute** restores the commit path after a muted stretch.
  - **raw callback** fires for every frame (loud + quiet) even while muted,
    reporting the per-frame buffer length and RMS.
  - **inactive guard**: frames before `active` are dropped entirely (no metering).
  - **sustained speech** appends each loud frame to the segment.
  - **stop()** tears down the speaking state.
  - NOTE: the mute test seeds `frameCount = 0` (the direct-drive harness skips
    `start()`, where the client initialises it; `undefined++` → NaN otherwise).

**Verification:**
- JS suite: `cd client && npm test` (`node --test`, node v22) → **20 passed,
  0 failed** (11 prior + 9 new).
- Full Python unit suite: `python -m pytest tests/unit/` → **2657 passed**
  (unchanged — no Python touched this lap).
- (`tests/test_session.py` still errors on collection — pre-existing missing
  `pipecat` dep, not a regression.)
...
1. **[latency, highest value] Pre-warm the capture pipeline** — still the top
   ...
   harness; cannot be replayed.
2. **[gain] Wire a `gainNode` into `ContinuousListener`** (default 1.0) — the
   iter-192 threshold×gain grid documents the operating envelope; now the JS
   suite (iter-194) can cover the new knob's parity + recovery behaviour.
3. **[debounce] Validate onset *timing* moves earlier at 100ms** before
   lowering the client's hard-coded 200ms — pairs with the now-wired pre-roll
...
## iter-198 — add a `max_first_onset_ms` worst-case onset-timing ceiling to the VAD sweep

**Branch:** `iter-198-onset-timing-worstcase` (merged ff to main, commit `edefbb2`)
**Date:** 2026-06-17

**Improvement (iter-197 follow-up):** iter-197 added `mean_first_onset_ms`
(`onset1`) so an onset-shaping knob's *average* timing effect is readable in one
sweep pass. But the mean hides the failure that matters most — a single
recording captured far too late. `SweepPoint` already carries that exact shape
on the *count* axis: `total_onsets` (aggregate) **plus** `min_onsets` (worst-case
floor, "one missed recording is a real miss"). The timing axis had only the mean
and no worst-case ceiling. This lap adds the missing companion, mirroring the
`min_onsets` design.

What changed:
- **`fixtures/replay_vad.py`** — `SweepPoint` gains a `max_first_onset_ms` field:
  the *latest* first-segment `onset_ms` across recordings that detected at least
  one segment. It is to `mean_first_onset_ms` what `min_onsets` is to
  `total_onsets` — the ceiling a sweep wants to *minimize*. Missed recordings are
  excluded on the same reasoning as the mean (no segment → no onset time);
  `0.0` when nothing detected. `aggregate_results` computes it
  (`max(first_onsets)`); `summary_line` and `_grid_summary_line` surface it as an
  `onset1_max=…ms` column in the human sweep/grid tables; it rides along in
  `--json` for free (dataclass field via `asdict`).
- **`tests/unit/test_replay_vad.py`** — +3 dedicated tests (67 → 70):
  `max_first_onset_is_latest_detected_recording` (equals `max` over the same
  detected onsets the mean averages; `>= mean`), `max_first_onset_excludes_missed_recordings`
  (only the detected recording counts at threshold 0.015), plus assertions
  folded into the empty-corpus zero test, the human-table column test
  (`onset1_max=` present), the grid human-table test (4 cells each carry
  `onset1=` and `onset1_max=`), and a new `test_sweep_json_includes_max_first_onset`
  (JSON payload includes the field, ceiling `>= mean`).
- **`docs/research/voice-capture-tuning.md`** — `onset1_max` documented in the
  "Each row reports" legend; new "Worst-case onset ceiling (iter-198)" subsection
  with real-corpus tables.

**Corpus evidence (the ceiling reveals what the mean hides),** threshold 0.006,
frame 1024, real 4-recording corpus:
- `--sweep debounce_ms 100,200,300`: lowering debounce 200→100ms pulls the
  *mean* ~261ms earlier (1532.5→1271.3ms) but `onset1_max` stays **pinned at
  2995.4ms** — the worst recording isn't debounce-limited, so a smaller debounce
  can't help its late opening. 300ms is the cliff (`trig=3/4`, ceiling rises to
  3343.7ms).
- `--sweep preroll_ms 0,256,512`: pre-roll moves *both* numbers in lockstep
  (mean 1532.5→1091.3ms, ceiling 2995.4→2484.5ms) — it shifts every detected
  recording's emitted onset uniformly, so it is the lever that tightens the
  tail. Takeaway: debounce improves the *typical* opening, pre-roll tightens the
  *worst case* — complementary, not redundant.

**Verification:**
- `python -m pytest tests/unit/test_replay_vad.py` → **70 passed** (67 + 3 new).
- Full Python unit suite: `python -m pytest tests/unit/` → **2666 passed**
  (2663 prior + 3 new).
- Integration `tests/integration/test_vad_recordings.py` → 12 skipped in the
  worktree (the binary corpus lives only in the main checkout); the CLI runs
  above exercised the new aggregate against the real corpus directly.
- (`tests/test_session.py` still errors on collection — pre-existing missing
  `pipecat` dep, not a regression.)
...
1. **[latency, highest value] Pre-warm the capture pipeline** — still the top
   ...
   harness; cannot be replayed.
2. **[debounce-default] Lower the client `debounceMs` default 200→100** — the
   mean-timing evidence supports it (iter-197), but iter-198 shows it does
   nothing for the worst-case opening; pair the default change with a pre-roll
   default bump so both the typical and tail openings improve. Gate on a
   busier/newly-synced corpus.
3. **[grid] Add a `--grid debounce_ms,preroll_ms` finding** in the research
   doc — both onset-timing axes are reachable as client knobs and both
   `onset1`/`onset1_max` aggregates now make the joint mean+tail timing effect
...
## iter-213 — WPM-mirroring decision seam (backlog #3)

**Branch:** `iter-213-wpm-mirror` (merged ff to main, commit `c40558c`)
**Date:** 2026-06-18

**Improvement (behavior — adapt the bot's TTS speed toward the user's rate):**
This lap deliberately breaks a **13-iteration run of near-identical
session-summary diversity-check sentinels** (iter-140..143, iter-208..212 — all
instances of the same `_emit_*_consistency_line` template) and picks up the
first *substantive behavioral* backlog item instead: **#3 in the ITERATION_LOG
"next directions" — adapt bot WPM toward the measured user WPM** (the iter-064
mirroring effect: matching the user's speaking rate raises rapport and lowers
the interruption rate).

It follows the codebase's established **decision-seam-first** rhythm
(iter-149/150/152/153/155/164): ship the pure, exhaustively-tested primitive
behind an off-by-default gate this lap; wire it into the live TTS path as a
separate, reviewable lap.

**Why now.** Three measurement surfaces already exist for the conversational
rhythm: iter-046 `bot_wpm` (the bot's measured rate), iter-064 `user_wpm` (the
user's) + the session-summary "Mirror gap: ±NN WPM (bot − user)" line, and
iter-210's `bot_wpm` consistency sentinel (fires when the rate is *consistently*
mis-set). Those three **measure** the gap; nothing yet **closes** it. This seam
is the decision that would — the natural follow-on iter-210's sentinel was
explicitly pointing at.

What changed:
- **`session/wpm_mirror.py`** (new, pure, dependency-free, file-path-loadable
  like its `turn_decider` / `backchannel_*` siblings):
  - `mirrored_speed(user_wpm, current_speed, config) -> float` — maps a measured
    user WPM to the next turn's Kokoro `speed`. Shape, in order:
    1. **Gate** — disabled config ⇒ return `current_speed` unchanged (no other
       input consulted).
    2. **No-measurement guard** — `user_wpm <= 0` (empty / zero-duration turn,
       the iter-064 guard) ⇒ return `current_speed`.
    3. **Ideal speed** — `ideal = user_wpm / base_wpm` (the speed that makes
       `bot_wpm` exactly match the user; `base_wpm=165` is the iter-046 green-band
       midpoint, recalibratable).
    4. **Damped nudge** — `target = current + strength*(ideal - current)`;
       `strength=0.5` converges over a few turns instead of lurching on a single
       noisy per-turn WPM (a one-word "yeah" reads as a very low rate). Same
       "react gradually to natural variation" discipline as the iter-115+
       sentinels.
    5. **Intelligibility clamp** — never produce a speed outside
       `[min_speed, max_speed]` (`[0.8, 1.3]`) no matter how extreme the
       measurement (40-WPM near-silence, 400-WPM burst).
    6. **Deadband** — if the clamped target differs from `current_speed` by less
       than `min_delta` (0.05), keep `current_speed` (no imperceptible churn the
       iter-210 sentinel would flag as needless variation).
    Monotone in `user_wpm` (a faster user never yields a slower bot) — the
    property a learned replacement would also hold.
  - `WpmMirrorConfig` (frozen) with `__post_init__` validation: positive
    `base_wpm`/`min_speed`, `strength` in `[0,1]`, `max_speed >= min_speed`,
    `min_delta >= 0`. A misconfigured mirror that silently races or drones is the
    worst failure mode, so bad tunables raise `ValueError` loudly (the same
    reasoning as `full_duplex.py`'s strict flag parsing).
  - `WpmMirror` — thin class wrapping the function so a future learned
    rate-matcher implements the identical `speed(user_wpm=…, current_speed=…)`
    interface without a call-site change. Stateless (the `strength` damping
    already smooths across turns), unlike `turn_decider`'s decider.
- **`tests/unit/test_wpm_mirror.py`** — 51 tests covering the full matrix:
  the **off-by-default invariant** (every input returns `current_speed`
  unchanged, even at full strength / exactly at `base_wpm`; `None`-config matches
  default); the no-measurement guard (non-positive `user_wpm`); the proportional
  nudge (faster speeds up / slower slows down / `user_wpm==base_wpm` is the fixed
  point; full vs partial strength; nudge from a non-unity current speed;
  monotonicity across a grid); the clamp (extreme fast→max, extreme slow→min,
  exact-boundary, grid never exceeds band); the deadband (sub-delta holds,
  at-delta applies, measured against current not ideal, zero disables);
  convergence (iterating walks to ideal within 1e-3, never overshoots); config
  validation (defaults, frozen, every guard + in-range OK); the `WpmMirror` class
  (disabled passthrough, matches the function, keyword-only `speed`, holds its
  config, stateless across calls); and purity (no config mutation).

**The off-by-default invariant is the whole point.** A default `WpmMirrorConfig`
has `enabled=False`, so `mirrored_speed` is the identity on `current_speed` for
every input — byte-for-byte today's fixed-rate behavior, and `user_wpm` isn't
even consulted. Wiring this into `mic_chat`'s TTS speed (the named follow-on) can
never regress the proven constant-speed path; the mirroring engages only behind
the explicit switch.

**No runtime behavior change.** The module is unwired — nothing in the live
`mic_chat` / `pipecat_server` path imports it yet, and even when wired the
default config makes it a passthrough.

**Verification:**
- `python -m pytest tests/unit/test_wpm_mirror.py` → **51 passed**.
- Full Python unit suite: `python -m pytest tests/unit/` → **2858 passed**
  (2807 prior + 51 new), run on the feature branch before ff-merge. GATE command:
  `cd ~/code-purp/geno-voice && python -m pytest tests/unit/`.
- `python -W error::SyntaxWarning -m py_compile session/wpm_mirror.py
  tests/unit/test_wpm_mirror.py` clean.
- (`tests/test_session.py` still errors on collection — pre-existing missing
  `pipecat` dep, not a regression.)

**Next planned items:**
1. **[wpm-mirroring, follow-on] Wire `WpmMirror` into the live TTS path.** Hold a
   `WpmMirror(config=…)` in `mic_chat.run_chat`, and after each turn's `user_wpm`
   is measured (iter-064), recompute the next turn's `speed` via
   `mirror.speed(user_wpm=…, current_speed=speed)`. The `speed` closure passed to
   `synthesize_with_alignment` / `build_audio_io` is currently a fixed `1.0`
   argument; making it a mutable per-turn value behind the off-by-default gate is
   the first lap that actually changes a live entrypoint's rate. Surface the
   adapted speed (and whether the mirror is active) in the session summary so it's
   measured, not asserted — the iter-210 `bot_wpm` sentinel should show the gap
   shrinking when it's on.
