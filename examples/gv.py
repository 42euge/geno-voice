...
def _best_stt_rtf_batch_row(rows):
    """Pick the single most-REPRESENTATIVE stt-rtf-batch engine (iter-413), or ``None``.

    The selection primitive behind ``--summary``: where ``--top-n`` (a COUNT) and
    ``--sort-by`` (an ORDERING) shape a TABLE the operator still reads, ``--summary``
    names the ONE engine that best speaks for the corpus — the engine whose median
    RTF sits CLOSEST to the corpus median, so an operator who just wants "which
    single transcriber represents the fleet's typical speed?" gets the answer
    without reading rows.

    Note the DIRECTION: this is the deliberate INVERSE of ``--sort-by delta``
    (iter-411), which floats the biggest corpus OUTLIERS to the top. The batch's
    job is to show which engines agree-vs-disagree on speed, so its two reductions
    sit at opposite ends — the sort surfaces who disagrees most, the summary
    surfaces who agrees most. "Best" is defined objectively and independently of
    ``--sort-by`` / ``--top-n``: among rows that carry a recommendation
    (``delta_from_median_rtf`` not ``None`` — i.e. a profiled engine with a
    per-corpus delta), the row with the SMALLEST |``delta_from_median_rtf``|
    (nearest the corpus median), ties broken toward the HIGHEST speed grade (the
    faster of two equally-central engines), and remaining ties broken by EARLIEST
    position in ``rows`` (so the pick is deterministic and prefers the engine the
    operator listed first). An unprofiled (no-sample) engine has
    ``delta_from_median_rtf`` ``None`` and is never picked. Returns ``None`` when no
    engine carries a delta (nothing to summarize — an empty or all-unprofiled
    corpus). Pure: reads the rows, mutates nothing.

    This is a genuinely new reduction — a single MIN, not a sort+truncate — so it
    is robust to whatever ``--sort-by`` / ``--top-n`` the operator did (or did not)
    request: the most-representative engine is the same regardless of how the table
    was ordered or capped. The STT-side analogue of :func:`_best_calib_batch_row`
    (which serves the calibration batch), with the engine's ``median_rtf`` / speed
    grade swapped in for the calibration batch's ``implied_base_wpm`` / dispersion
    grade.
    """
    candidates = [r for r in rows if r["delta_from_median_rtf"] is not None]
    if not candidates:
        return None
    # Nearest the median first (|Δ| ascending), then highest grade (negative rank →
    # descending); ``min`` over this key is stable so earliest position breaks
    # remaining ties. An unprofiled engine never reaches here (filtered above), but
    # the ``.get(..., -1)`` default keeps the key total-ordered defensively.
    return min(
        candidates,
        key=lambda r: (
            abs(r["delta_from_median_rtf"]),
...
def _format_stt_rtf_batch_summary_verdict(row):
    """One human line naming the single most-representative engine (iter-413).

    The ``--summary`` verdict body: given the row :func:`_best_stt_rtf_batch_row`
    picked, spells the corpus consensus as ``representative: <engine> → <n> RTF
    (grade <grade>, Δmedian <±n>)``. The signed Δmedian is shown so the reader can
    confirm the pick really is near the corpus centre (it should be the smallest
    |Δ| in the table). Pure: returns a single string; the caller owns the
    surrounding lines. The STT-side twin of
    :func:`_format_calib_batch_summary_verdict`.
    """
    profile = row["profile"]
    sign = _signed_float3(row["delta_from_median_rtf"])
    return (
        f"  representative: {row['engine']} → {profile.median_rtf:.3f} RTF "
        f"(grade {profile.speed_grade}, Δmedian {sign})"
    )
...
def render_stt_rtf_batch(batch, *, min_grade=None, sort_by=None, top_n=None,
    ...
    ``summary`` (iter-413) collapses the whole table to ONE verdict line naming the
    most-representative engine (:func:`_best_stt_rtf_batch_row` — the engine nearest
    the corpus median RTF), the STT-side analogue of the iter-402 ``summary`` on
    ``calibrate-base-wpm-batch``. The pick is INDEPENDENT of ``--sort-by`` /
    ``--top-n`` (it is an objective reduction, not a row off the top of a sort) but
    respects ``--min-grade`` (the pick is made over the surviving engines, so a
    floor scopes who can represent the corpus). The whole-corpus summary +
    ...
    def _corpus_lines():
        ...
        # every render variant (full table, empty-floor note, and --summary
        # verdict) so the corpus consensus the rows are measured against is always
        # visible. Computed over the WHOLE corpus, so it is unaffected by
        ...
        if batch.num_profiled:
            ...
            out.append("  corpus: (no engine profiled — nothing to summarise)")
        counts = batch.grade_counts
        grades = ", ".join(
    ...
    if summary:
        ...
        if best is None:
            ...
            lines.append(_format_stt_rtf_batch_summary_verdict(best))
        lines.extend(_corpus_lines())
        return lines
...
def render_stt_rtf_batch_json(batch, *, min_grade=None, sort_by=None, top_n=None,
    ...
    ``summary`` (iter-413) replaces the ``rows`` list with a single ``best`` key
    holding the most-representative engine (:func:`_best_stt_rtf_batch_row` —
    nearest the corpus median RTF), or ``null`` when no engine carries a delta. The
    ``best`` object is the SAME per-engine shape a ``rows`` entry uses
    (:func:`_stt_rtf_batch_row_obj`), so it carries a ``delta_from_median_rtf``. A
    top-level ``summary: true`` flags the mode. The pick is INDEPENDENT of
    ...
    if summary:
        best = _best_stt_rtf_batch_row(kept_rows)
        payload["best"] = None if best is None else _stt_rtf_batch_row_obj(best)
    else:
        payload["rows"] = [
            _stt_rtf_batch_row_obj(r)
            for r in _truncate_batch_rows(
                _sort_stt_rtf_batch_rows(kept_rows, sort_by),
                top_n,
            )
        ]
    return json.dumps(payload, indent=2)
...
def render_stt_rtf_batch_csv(batch, *, min_grade=None, sort_by=None, top_n=None,
    ...
    ``summary`` (iter-413) emits just the header plus the ONE most-representative
    engine row (:func:`_best_stt_rtf_batch_row` — nearest the corpus median RTF) —
    header-only when no engine carries a delta. The column set is unchanged, so a
    summary CSV unions cleanly with a full batch — it is simply the single best
    row. The pick is INDEPENDENT of ``sort_by`` / ``top_n`` (those grid-shaping
    comments are omitted in summary mode) but respects ``min_grade``; a leading
    ...
    if summary:
        ...
        data_rows = _truncate_batch_rows(
            _sort_stt_rtf_batch_rows(kept_rows, sort_by), top_n
        )
    buf = io.StringIO()
    writer = csv.writer(buf)
    writer.writerow(
...
def _format_knob_grid_summary_verdict(cell, row_axis, col_axis, biases):
    """One human line naming the single best grid CELL (iter-383).

    The 2-D analogue of :func:`_format_knob_summary_verdict`: where the 1-D sweep
    verdict names one swept value, the grid verdict names the (gate × column) CELL
    :func:`_best_knob_row` picked, spelling the adopt-this-setting call as ``best:
    <row_axis>=<value> <col_axis>=<value> → --min-silence-ms <n> [bias] (confidence
    <grade>, spread <s>ms)``. Both axis labels precede the recommendation because a
    cell varies in TWO dimensions (the only structural difference from the sweep
    line — exactly how :func:`render_vad_gap_recommend_knob_grid`'s body row carries
    two leading columns where the sweep carries one). The headline bias is the LAST
    of the selected ``biases`` in canonical short..balanced..long order (default
    triad → ``long``, ``--bias short`` → ``short``), matching the table's rightmost
    recommendation column. ``row_axis`` / ``col_axis`` name the two swept dimensions
    (rendered via :func:`_format_sweep_axis_value`). Pure: returns a single string;
    the caller owns the surrounding lines.
    """
    bias = list(biases)[-1]
    rec = _format_cut_label(_knob_sweep_bias_ms(cell, bias))
    row_value = _format_sweep_axis_value(row_axis, cell[row_axis])
    col_value = _format_sweep_axis_value(col_axis, cell[col_axis])
    ...
    return (
        f"  best: {row_label}={row_value} {col_label}={col_value} → "
        f"--min-silence-ms {rec} [{bias}] "
        f"(confidence {cell['grade']}, spread {spread}ms)"
    )
...
def _truncate_knob_rows(rows, top_n):
    """Keep only the first ``top_n`` knob-sweep rows (iter-380), most-useful first.

    The count companion of :func:`_filter_knob_rows_by_grade` (a THRESHOLD) and
    :func:`_sort_knob_rows` (an ORDERING). Where ``--min-grade`` keeps every row at
    or above a confidence floor (an unknown number of rows), ``--top-n`` keeps a
    FIXED COUNT — "show me the N most trustworthy knob settings" — so it composes
    naturally with ``sort_by`` and is applied AFTER it: the N kept rows are the N
    that sorted to the top. With ``top_n`` ``None`` (no cap requested) returns the
    rows unchanged (a copy, so a sweep with no ``--top-n`` is byte-identical to the
    pre-cap output). A ``top_n`` larger than the row count keeps every row (slicing
    is saturating, never raises). Pure: never mutates the source list. The caller
    guarantees ``top_n >= 1`` via :func:`positive_int_type` at the parser, so a cap
...
def render_vad_gap_recommend_knob_sweep(
    ...
    ``top_n`` (iter-380) keeps only the first ``top_n`` rows AFTER the ``sort_by``
    ordering, so ``--sort-by grade --top-n 3`` shows the THREE most-trustworthy knob
...
def render_vad_gap_recommend_knob_sweep_json(
    ...
    ``top_n`` (iter-380) keeps only the first ``top_n`` ``sweep`` rows AFTER the
    ``sort_by`` ordering. When a cap is set the payload also carries a top-level
...
def render_vad_gap_recommend_knob_sweep_csv(
    ...
    ``top_n`` (iter-380) keeps only the first ``top_n`` data rows AFTER the
...
def render_vad_gap_recommend_knob_grid(
    ...
    *,
    ...
    ``sort_by`` (iter-379) reorders the kept cells so the most-useful region of the
    plane reads first instead of by row-major (gate × column) order: ``"grade"`` =
    most-trustworthy first (confidence descending), ``"spread"`` = most-decisive
    first (short→long spread ascending). It is applied AFTER ``min_grade``, so only
    surviving cells are reordered. The default ``None`` keeps the row-major order
    ...
    ``summary`` (iter-383) collapses the whole grid to ONE verdict line naming the
    single best (gate × column) CELL (:func:`_best_knob_row` — highest confidence,
    ties to the tightest spread), so an operator who just wants "which cell do I
    adopt?" gets the answer without reading the grid. It is applied to the cells that
    survive ``min_grade`` and is INDEPENDENT of ``sort_by`` / ``top_n`` (the best
    cell is the best regardless of how — or whether — the grid was ordered or
    capped). When no surviving cell carries a recommendation a single ``(no grid
    cell ...)`` note is emitted instead. The default ``False`` renders the full grid.
    The grid analogue of the iter-382 ``summary`` on the 1-D knob sweep, reusing the
    same :func:`_best_knob_row` selection primitive.
    """
    if any(r is None for r in results):
        return [
            "silero VAD unavailable: install 'silero-vad' (pulls torch + "
    ...
    col_label = _SWEEP_AXIS_LABEL.get(col_axis, col_axis)
    if summary:
        # The verdict is an objective reduction over the grade-filtered cells —
        # order/count-independent, so --sort-by / --top-n do not change the pick.
        kept = _filter_knob_rows_by_grade(
        ...
        best = _best_knob_row(kept)
        lines = [
            f"silero VAD recommended-hangover knob grid summary — {name} "
        ...
        if best is None:
            ...
            lines.append(f"  (no grid cell{floor} carries a recommendation)")
            return lines
        lines.append(
            _format_knob_grid_summary_verdict(best, row_axis, col_axis, biases)
        )
        return lines
...
def render_vad_gap_recommend_knob_grid_json(
    ...
    *,
    ...
    ``sort_by`` (iter-379) reorders the ``grid`` cells (``"grade"`` = confidence
    DESCENDING, ``"spread"`` = short→long spread ASCENDING), applied AFTER the
    ``min_grade`` filter so only surviving cells are reordered. When a key is set the
    payload also carries a top-level ``sort_by`` key naming it, so a consumer knows
    ...
    ``summary`` (iter-383) replaces the ``grid`` list with a single ``best`` key
    holding the one best grid cell (:func:`_best_knob_row` — highest confidence, ties
    to the tightest spread) or ``null`` when no surviving cell carries a
    recommendation, and sets ``summary`` true. The pick is computed over the
    ``min_grade``-filtered cells and is INDEPENDENT of ``sort_by`` / ``top_n``. The
    ``best`` cell is narrowed by ``biases`` like the grid cells, so a consumer reads
    the same columns. The default ``False`` emits the full ``grid`` list. The grid
    analogue of the iter-382 ``summary`` on the 1-D knob sweep.
    """
    if any(r is None for r in results):
        return json.dumps(
            {
...
def render_vad_gap_recommend_knob_grid_csv(
    ...
    *,
    ...
    ``sort_by`` (iter-379) reorders the data rows (``"grade"`` = confidence
    DESCENDING, ``"spread"`` = short→long spread ASCENDING), applied AFTER the
    ...
    ``summary`` (iter-383) emits just the header plus the ONE best grid cell
    (:func:`_best_knob_row`) — header-only when no surviving cell carries a
    recommendation. The column set is unchanged, so a summary CSV unions cleanly
    with a full grid — it is simply the single best cell. The pick is computed over
    the ``min_grade``-filtered cells and is INDEPENDENT of ``sort_by`` / ``top_n``.
    The default ``False`` emits every kept cell. The grid analogue of the iter-382
    ``summary`` on the 1-D knob sweep.
    """
    if any(r is None for r in results):
        return (
            "# silero VAD unavailable: install 'silero-vad' (pulls torch + "
    ...
    if summary:
        ...
        best = _best_knob_row(kept)
        cells = [best] if best is not None else []
    else:
        cells = _truncate_knob_rows(
            _sort_knob_rows(
                _filter_knob_rows_by_grade(
...
def render_vad_gap_recommend_batch(results, labels, *, bias=DEFAULT_GAP_RECOMMEND_BIAS,
    ...
    ``min_grade`` (iter-389) drops every recording row whose confidence ``grade`` is
    below the named floor (``"weak"`` / ``"moderate"`` / ``"strong"``), leaving only
    the recordings whose recommendation is trustworthy enough to act on; rows graded
    ``"none"`` or ungraded (<2 segments) are always dropped when a floor is set. It is
    applied FIRST (before ``sort_by`` / ``top_n`` / ``summary``), so every later stage
    sees only the surviving recordings — but the corpus aggregates (median / min / max
    / spread / counts) are still computed over the WHOLE corpus, so the floor narrows
    which recordings you READ without redefining what the corpus consensus IS. The
    batch analogue of the iter-376 ``min_grade`` on the knob sweep. The default
    ``None`` keeps every recording (byte-identical to the pre-filter render); when the
    filter removes every recording a single ``(no recording meets ...)`` note replaces
    the body.
    ...
    capped) but respects ``min_grade`` (the representative is picked among recordings
    that clear the floor). When no recording carries a recommendation a single ``(no
    recording ...)`` note is emitted instead. The default ``False`` renders the full
    table.
    """
    if any(r is None for r in results):
        return [
    ...
    # The grade floor is applied first: every later stage (summary / sort / top-n)
    # sees only the surviving recordings, while the corpus aggregates below stay
    # computed over the WHOLE corpus.
    kept_rows = _filter_knob_rows_by_grade(d["rows"], min_grade)
    if summary:
        # The verdict is an objective reduction over the grade-filtered rows —
        # order/count-independent, so --sort-by / --top-n do not change the pick.
        best = _best_batch_row(kept_rows)
        lines = [
        ...
        if best is None:
            ...
            lines.append(
                f"  (no recording{floor} carries a recommendation — every WAV has "
    ...
    rows = _truncate_batch_rows(_sort_batch_rows(kept_rows, sort_by), top_n)
    rec_width = max([len("recording")] + [len(str(label)) for label in labels])
    bias_line = f"  bias: {d['bias']}"
    if min_grade is not None:
        bias_line += f"  (min grade: {min_grade})"
    if sort_by is not None:
        bias_line += f"  (sorted by: {sort_by})"
    if top_n is not None and len(rows) < len(kept_rows):
        bias_line += f"  (top {len(rows)} of {len(kept_rows)})"
    lines = [
    ...
    if min_grade is not None and not rows:
        lines.append(
            f"  (no recording reaches confidence grade '{min_grade}' or better)"
        )
        return lines
    for row in rows:
...
def render_vad_gap_recommend_batch_json(results, labels, *,
    ...
    ``min_grade`` (iter-389) drops every ``rows`` entry whose confidence ``grade`` is
    below the named floor, applied FIRST (before ``sort_by`` / ``top_n`` / ``summary``)
    so every later stage sees only the surviving recordings. When set the payload also
    carries a top-level ``min_grade`` key naming the floor, so a consumer knows the
    ``rows`` list was filtered. The corpus aggregates (median / min / max / spread /
    counts) remain computed over the WHOLE corpus and are unaffected. The default
    ``None`` keeps every recording (unchanged payload).
    ...
    is INDEPENDENT of ``sort_by`` / ``top_n`` but respects ``min_grade`` (chosen among
    recordings that clear the floor); the corpus aggregates (median / min / max /
    spread / counts) are still carried so a consumer can see what the representative
    recording is central WITHIN. The default ``False`` carries the full ``rows`` list.
    """
    if any(r is None for r in results):
        return json.dumps(
            {
    ...
    d = vad_gap_recommend_batch(results, labels, bias=bias)
    # Grade floor first: every later stage sees only surviving recordings, while the
    # corpus aggregates carried below stay computed over the WHOLE corpus.
    kept_rows = _filter_knob_rows_by_grade(d["rows"], min_grade)
    if summary:
        best = _best_batch_row(kept_rows)
        d.pop("rows")
        ...
        payload["best"] = best
        return json.dumps(payload, indent=2)
    d["rows"] = _truncate_batch_rows(_sort_batch_rows(kept_rows, sort_by), top_n)
...
def render_vad_gap_recommend_batch_csv(results, labels, *,
    ...
    ``min_grade`` (iter-389) drops every data row whose confidence ``grade`` is below
    the named floor, applied FIRST (before ``sort_by`` / ``top_n`` / ``summary``). The
    header and column set are unchanged (a filtered run still unions cleanly with a
    full one — it just has fewer rows), so the floor is encoded purely in which rows
    appear. The default ``None`` keeps every recording.
    ...
    The pick is INDEPENDENT of ``sort_by`` / ``top_n`` but respects ``min_grade``. The
    default ``False`` emits every row.
    """
    if any(r is None for r in results):
        return (
            "# silero VAD unavailable: install 'silero-vad' (pulls torch + "
    ...
    d = vad_gap_recommend_batch(results, labels, bias=bias)
    # Grade floor first, so summary / sort / top-n all see only surviving recordings.
    kept_rows = _filter_knob_rows_by_grade(d["rows"], min_grade)
    if summary:
        best = _best_batch_row(kept_rows)
        rows = [best] if best is not None else []
...
def vad_gap_peak_sweep(
    ...
    *,
    ...
    iter-368 adds ``top_n`` — the iter-354 single-shot ranking, carried per swept
    value. Each row gains a ``peaks`` list of up to ``top_n`` ranked band objects
    (descending rate, earliest band first on a tie), each with a 1-based ``rank``,
    so an operator can watch how the WHOLE steepest-N ranking reorders as the
    segmenter knob tightens — not just whether the single steepest band moves. A
    knob change that swaps which cluster is #1 vs #2 (without moving the headline
    ``peak_rate_per_100ms`` much) is invisible to the scalar columns but obvious in
    the ranked list. The scalar ``peak_*`` fields are unchanged and always echo the
    single steepest band (``peaks[0]``), so the default ``top_n=1`` row is a strict
    superset of the iter-364/366 shape (its ``peaks`` then holds exactly one entry,
    or is empty for a no-peak row). ``top_n`` is threaded straight into
...
def render_vad_gap_peak_sweep(
    ...
    *,
    ...
    With ``top_n > 1`` (iter-368) each row is followed by an indented ranked
    sub-block — the iter-354 single-shot ``--top-n`` view, per swept value: up to
    ``top_n`` numbered lines naming the N steepest bands (descending rate, earliest
    first on a tie), so an operator watches how the WHOLE ranking reorders as the
    knob tightens, not just whether the single steepest band moves. A row that
    names no cost peak prints a short "(no cost peak)" note instead. The default
    ``top_n=1`` leaves the table byte-for-byte unchanged (the scalar peak columns
    ...
    for row in rows:
        ...
        if top_n > 1:
            ...
            # when the range holds fewer non-empty bands), or a "(no cost peak)"
            # note for a no-peak row.
            if not row["peaks"]:
                lines.append("      top bands: (no cost peak)")
            else:
                lines.append(f"      top {top_n} costliest bands (steepest first):")
                for pk in row["peaks"]:
                    lo = _format_cut_label(pk["from_ms"])
                    hi = _format_cut_label(pk["to_ms"])
                    width = _format_cut_label(pk["width_ms"])
                    lines.append(
                        f"        #{pk['rank']}: {lo}-{hi}ms (width {width}ms) — "
                        f"merges +{pk['merged_added']} pauses, "
                        f"{pk['rate_per_100ms']:.3f} per +100ms"
                    )
        if show_rate_dist:
            # iter-366: the observed non-empty band-rate distribution at this
...
def render_vad_gap_peak_sweep_json(values, results, *, name, cuts_ms=DEFAULT_GAP_CDF_CUTS_MS, axis="threshold", top_n=1, min_rate=0.0, min_rate_pct=None, rate_pcts=DEFAULT_BAND_RATE_PCTS):
    ...
    label the quantiles without re-deriving them.

    iter-368: like the single-shot JSON (which always carries ``peaks`` /
    ``top_n``), every sweep row carries a ``peaks`` list of up to ``top_n`` ranked
    band objects (descending rate, each with a 1-based ``rank``) regardless of
    ``--top-n`` — so a machine consumer always reads the full ranking, with the
...
def vad_gap_peak_grid(
    ...
    *,
    ...
    distribution (min/mean/max gap) across the cartesian product of TWO knobs,
    this surface tabulates the iter-350 cost PEAK per cell — for each (row, col)
    pair it runs :func:`vad_gap_peak` (single peak, ``top_n=1``) over that cell's
    segmentation and records the steepest band: its ms range, the pauses it
    merges, and its marginal rate per +100 ms of hangover. ``vad-gap-grid`` is to
    ``vad-gap-sweep`` what this is to ``vad-gap-peak-sweep``: same row-major
    flattening, but the headline column is now ``peak_rate_per_100ms`` (the cost
    of pushing the hangover through the densest pause cluster) read across two
    knobs at once instead of one 1-D peak sweep at a time. Where the gap grid
    watches the cheapest pause floor move in two dimensions, this watches the
    costliest cluster move — the cell where NOT to push the hangover through.
    ...
    :func:`vad_segmentation_grid` / :func:`vad_gap_grid` consume. ``cuts_ms`` (the
    candidate hangover cuts defining the cost bands) is shared across every cell
    so the bands are scanned on the same axis everywhere.
    ...
    (iter-354 / iter-368 / this lap). Each cell gains a ``peaks`` list of up to
    ``top_n`` ranked band objects (descending rate, earliest band first on a tie),
    each with a 1-based ``rank``, so an operator can watch how the WHOLE
    steepest-N ranking reorders across TWO knobs at once — not just whether the
    single steepest band moves. A knob change that swaps which cluster is #1 vs #2
    (without moving the headline ``peak_rate_per_100ms`` much) is invisible to the
    scalar columns but obvious in the ranked list. The scalar ``peak_*`` fields are
    unchanged and always echo the single steepest band (``peaks[0]``), so the
    default ``top_n=1`` cell is a strict superset of the iter-365/367 shape (its
    ``peaks`` then holds exactly one entry, or is empty for a no-peak cell).
    ``top_n`` is threaded straight into :func:`vad_gap_peak`, so each cell's
    ranking agrees EXACTLY with ``gv vad-gap-peak --top-n`` at the matching cell.
...
def render_vad_gap_peak_grid(
    ...
    *,
    ...
    :func:`render_vad_gap_peak_sweep`: a FLAT one-row-per-cell table (not a
    matrix) so each cell's two swept values plus the costliest band's measures
    stay unambiguous. ``name`` is the WAV being swept; ``row_axis`` / ``col_axis``
    name the two swept dimensions (set the two leading column labels and value
    formats). Any ``None`` in ``results`` (segmenter unavailable) yields the
    ...
    the costliest band's ms range / merged-pauses / rate per +100 ms; a cell that
    names no cost peak (no gaps, or an all-valley range) prints ``-`` in the peak
    columns (distinct from a band with a ``0.000`` rate, which never happens — a
    ...
    With ``top_n > 1`` (iter-369) each cell is followed by an indented ranked
    sub-block — the iter-354 single-shot ``--top-n`` view, per grid cell: up to
    ``top_n`` numbered lines naming the N steepest bands (descending rate, earliest
    first on a tie), so an operator watches how the WHOLE ranking reorders across
    two knobs at once, not just whether the single steepest band moves. A cell that
    names no cost peak prints a short "(no cost peak)" note instead. The default
    ``top_n=1`` leaves the table byte-for-byte unchanged (the scalar peak columns
    already carry the single steepest band). The ranked block and the
    ...
    for cell in cells:
        if not cell["peak_found"]:
            ...
            band = (
                f"{_format_cut_label(cell['peak_from_ms'])}-"
                f"{_format_cut_label(cell['peak_to_ms'])}"
            )
            peak_cols = (
                f"{band:>12}  {cell['peak_merged_added']:>6}  "
                f"{cell['peak_rate_per_100ms']:>10.3f}"
            )
        lines.append(
            f"  {_format_sweep_axis_value(row_axis, cell[row_axis]):>11}  "
            f"{_format_sweep_axis_value(col_axis, cell[col_axis]):>11}  "
        ...
        if top_n > 1:
            ...
            # the range holds fewer non-empty bands), or a "(no cost peak)" note
            # for a no-peak cell.
            if not cell["peaks"]:
                lines.append("      top bands: (no cost peak)")
            else:
                lines.append(f"      top {top_n} costliest bands (steepest first):")
                for pk in cell["peaks"]:
                    lo = _format_cut_label(pk["from_ms"])
                    hi = _format_cut_label(pk["to_ms"])
                    width = _format_cut_label(pk["width_ms"])
                    lines.append(
...
def render_vad_gap_peak_grid_json(
    row_values,
    col_values,
    results,
    *,
    ...
    cells vary (the cells are keyed by those same names), the ``cuts_ms`` axis the
    bands were scanned over, and a ``grid`` list of per-cell rows, each with
    ``num_segments`` / ``num_gaps`` / ``peak_found`` / ``peak_from_ms`` /
    ``peak_to_ms`` / ``peak_width_ms`` / ``peak_merged_added`` /
    ``peak_rate_per_100ms`` (the peak fields ``null`` / ``false`` for a cell that
    names no cost peak, the same JSON spelling :func:`render_vad_gap_peak_sweep_json`
    ...
    iter-369: like the single-shot JSON and the sweep's
    ``render_vad_gap_peak_sweep_json`` (which always carry ``peaks`` / ``top_n``),
    every grid cell carries a ``peaks`` list of up to ``top_n`` ranked band objects
    (descending rate, each with a 1-based ``rank``) regardless of ``--top-n`` — so
    a machine consumer always reads the full ranking, with the requested count
...
def render_vad_gap_peak_grid_csv(
    row_values,
    col_values,
    results,
    *,
    ...
    ``<row_axis>,<col_axis>,num_segments,num_gaps,peak_found,peak_from_ms,
    peak_to_ms,peak_width_ms,peak_merged_added,peak_rate_per_100ms`` table (one
    row per cell, in row-major order) that pivots straight into a spreadsheet or a
    plotting script. The first two column headers are the swept axis names so the
    grid is self-describing. ``name`` is accepted for signature parity with the
    other ``render_vad_gap_peak_grid_*`` twins but is not part of the tabular body
    (a CSV is a pure data grid), matching :func:`render_vad_gap_peak_sweep_csv`. A
    cell that names no cost peak emits ``peak_found`` ``False`` and empty cells in
    the peak-measure columns (the CSV spelling of JSON ``null`` / the human
...
def cmd_stt_rtf_batch(args, *, log=print):
    ...
    human, ``--json``, and ``--csv`` output. iter-413 adds ``--summary`` to collapse
    the table to the ONE most-representative engine (nearest the corpus median RTF,
...
def cmd_vad_gap_recommend_knob_sweep(args, *, log=print, segmenter=None, availability=None):
    ...
    sort_by = getattr(args, "sort_by", None)
    # iter-380 --top-n count cap: keep only the first N rows AFTER --sort-by, so
...
def cmd_vad_gap_recommend_knob_grid(args, *, log=print, segmenter=None, availability=None):
    ...
    # iter-379 --sort-by ordering: reorder the kept cells (grade DESC / spread ASC)
    # so the most-useful region of the plane reads first (the grid analogue of the
    ...
    # iter-383 --summary: collapse the whole grid to ONE verdict naming the single
    # best (gate × column) cell (the grid analogue of the iter-382 knob-sweep
    # --summary). Independent of --sort-by/--top-n; composes with --min-grade/--bias.
    summary = getattr(args, "summary", False)
...
def cmd_vad_gap_peak_sweep(args, *, log=print, segmenter=None, availability=None):
    ...
    iter-368 adds ``--top-n``: the iter-354 single-shot ranking, carried per swept
    value. ``--top-n N`` appends an indented numbered sub-block under each human
    row naming the N steepest bands; the ``--json`` face always carries the ranking
    per row as ``peaks`` (with the requested count echoed once at top level as
    ...
    show_rate_dist = getattr(args, "show_rate_dist", False)
    rate_pcts = getattr(args, "rate_pcts", DEFAULT_BAND_RATE_PCTS)
    # iter-368: --top-n drives the iter-354 ranking for the human (numbered
    # sub-block under each row) AND json (per-row `peaks`) faces; the CSV's flat
...
def cmd_vad_gap_recommend_batch(args, *, log=print, segmenter=None, availability=None):
    ...
    ``--bias`` (iter-351) is shared too. ``--min-grade`` (iter-389) drops every
    recording whose confidence is below a trust floor BEFORE the table/summary (the
    corpus aggregates still cover the whole corpus); ``--sort-by`` (iter-386) orders
    the surviving rows most-useful-first; ``--top-n`` (iter-387) then keeps only the N
    most-useful recordings (applied AFTER ``--sort-by``); ``--summary`` (iter-388)
    collapses the corpus to ONE verdict naming the single most-representative
    recording (nearest the corpus median), independent of ``--sort-by`` / ``--top-n``
    but honouring ``--min-grade``.
    ``--csv`` is mutually exclusive with ``--json``; when ``silero-vad`` is absent
    the handler prints the install hint and returns, never crashing.
    """
    if segmenter is None or availability is None:
    ...
    if not availability():
        ...
        if as_json:
            log(render_vad_gap_recommend_batch_json(
                results, labels, bias=bias, min_grade=min_grade, sort_by=sort_by,
...
def cmd_vad_gap_peak_grid(args, *, log=print, segmenter=None, availability=None):
    """Segment one WAV across a 2-D knob grid and tabulate the COSTLIEST cost band.

    iter-365's peak-side analogue of :func:`cmd_vad_gap_grid`, and the 2-D
    analogue of :func:`cmd_vad_gap_peak_sweep`. Where ``gv vad-gap-grid``
    tabulates the inter-segment silence-gap distribution (min/mean/max gap) per
    cell of a gate × column-knob grid, ``gv vad-gap-peak-grid`` tabulates the
    iter-350 cost PEAK per cell — for each (row, col) it names the steepest
    cost-curve band (the densest pause cluster, the most expensive place to raise
    the end-of-turn hangover) and reports its ms range, merged pauses, and rate
    per +100 ms. The headline is how the peak rate MOVES across two knobs at once:
    where ``vad-gap-grid`` watches the cheapest pause floor move in two dimensions,
    this watches the costliest cluster move — the cell where NOT to push the
    hangover through. It is to ``vad-gap-peak-sweep`` what ``vad-gap-grid`` is to
    ``vad-gap-sweep``.

    Mirrors :func:`cmd_vad_gap_grid`'s axis layout: the ROW axis is always the
    P(speech) gate (``--thresholds``); the COLUMN axis is ``--min-silences`` (the
    trailing-silence hangover, the default), ``--min-speeches`` (the
    minimum-speech floor), ``--speech-pads`` (the symmetric region padding), or
    ``--max-speeches`` (the force-split ceiling, in SECONDS) — mutually exclusive,
    exactly one column axis per run. Whichever knob is NOT the column axis is held
    fixed at its scalar; every other knob is shared across all cells. ``--cuts-ms``
    (the candidate hangover cuts defining the cost bands) is shared across every
    cell so the bands are scanned on the same axis everywhere. Like
    ``vad-gap-grid`` there is no ``--target`` pick block (the pick scores on
    segment count, which the peak surface does not headline).
    ...
    iter-369 adds ``--top-n``: the iter-354 single-shot top-N ranking, carried per
    grid cell (mirroring :func:`cmd_vad_gap_peak_sweep`'s per-row iter-368 view,
    completing the top-N view across the single-shot / sweep / grid trio).
    ``--top-n N`` appends an indented numbered ranked sub-block under each human
    ...
    show_rate_dist = getattr(args, "show_rate_dist", False)
    rate_pcts = getattr(args, "rate_pcts", DEFAULT_BAND_RATE_PCTS)
    # iter-369: --top-n drives the per-cell top-N ranking for the human AND json
    # faces (the human gets the numbered sub-block; the json always carries
    ...
    # Row-major: row 0's whole row of columns first, then row 1's, … — the same
    # order vad_gap_peak_grid flattens into.
    results = [_seg(rv, cv) for rv in row_values for cv in col_values]
    name = results[0].name if results else args.wav
    if as_json:
        log(
...
DEFAULT_HANDLERS = {
    ...
    "vad-gap-peak-grid": cmd_vad_gap_peak_grid,
}
...
def build_parser():
    ...
    stt_rtf_batch.add_argument(
        "--summary",
        action="store_true",
        help="Collapse the table to the ONE most-representative engine (nearest "
    ...
    vad_gap_rec_knob_sweep.add_argument(
        ...
        default=None,
        dest="top_n",
        help="Keep only the first N swept rows AFTER --sort-by, e.g. "
    ...
    vad_gap_rec_knob_grid.add_argument(
        ...
        default=None,
        dest="sort_by",
        help="Reorder the grid cells so the most-useful region reads first: "
        "'grade' = most-trustworthy first (confidence descending), 'spread' = "
        "most-decisive first (short..long spread ascending); applied after "
    ...
    vad_gap_rec_knob_grid.add_argument(
        "--summary",
        action="store_true",
        help="Collapse the grid to ONE verdict naming the single best (gate × "
        "column) cell (highest confidence, ties to the tightest spread) — answers "
        "'which cell do I adopt?' without reading the grid; independent of "
        "--sort-by/--top-n, composes with --min-grade/--bias (default: full grid)",
    )
    ...
    vad_gap_peak_sweep.add_argument(
        ...
        default=1,
        dest="top_n",
        help="Name the N steepest cost bands under EACH swept row instead of just "
        "the single peak (ranked by descending rate, earliest band first on a "
        "tie; only non-empty bands count, so fewer than N may appear) — the "
        "iter-354 single-shot --top-n view, per swept value, so you can watch how "
    ...
    vad_gap_rec_batch.add_argument(
        ...
        default=None,
        dest="min_grade",
        help="Drop recordings whose confidence grade is below this floor "
        "(weak/moderate/strong) before the table/summary, leaving only the "
        "recordings worth acting on; the corpus median / spread is still computed "
        "over the whole corpus (default: keep every recording)",
    )
    vad_gap_rec_batch.add_argument(
        "--sort-by",
    ...
    # gv vad-gap-peak-grid — segment one WAV across a 2-D knob grid and tabulate
    # the COSTLIEST cost-curve band per cell (iter-365). The peak-side analogue
    # of vad-gap-grid (and the 2-D analogue of vad-gap-peak-sweep): rows are
    # always the P(speech) gate (--thresholds); the column axis is the
    # trailing-silence hangover (--min-silences, default, ms), the minimum-speech
    # floor (--min-speeches, ms), the region padding (--speech-pads, ms), or the
    # force-split ceiling (--max-speeches, SECONDS), mutually exclusive. Each cell
    # names the steepest band (densest pause cluster) so an operator can read
    # where the cost of raising --min-silence-ms through it MOVES across two knobs
    # at once. Shares the --cuts-ms cost-band axis with vad-gap-peak(-sweep). No
    # --target pick block (the pick scores on segment count, which the peak
    # surface does not headline).
    vad_gap_peak_grid = sub.add_parser(
        "vad-gap-peak-grid",
        help="Offline Silero VAD — segment a WAV across a 2-D grid (gate "
        "--thresholds × a column axis: --min-silences hangover, --min-speeches "
        "floor, --speech-pads region padding (ms), or --max-speeches force-split "
        "ceiling (seconds)) and tabulate the costliest cost-curve band per cell "
        "(the densest pause cluster — watch how the cost of raising "
        "--min-silence-ms through it moves in two dimensions)",
    )
    ...
    # The column axis: --min-silences (default) OR --min-speeches OR
    # --speech-pads OR --max-speeches, never more than one. The default list
    # lives on --min-silences so a bare `vad-gap-peak-grid rec.wav` sweeps gate ×
    # hangover; a group default isn't "provided", so the mutex only fires when
    # two are passed explicitly. Mirrors vad-gap-grid's column mutex exactly.
    vad_gap_peak_grid_col = vad_gap_peak_grid.add_mutually_exclusive_group()
    vad_gap_peak_grid_col.add_argument(
        "--min-silences",
        type=nonneg_float_list_type,
        default=[400.0, 600.0, 800.0, 1000.0],
        dest="min_silences",
    ...
    vad_gap_peak_grid.add_argument(
        ...
        dest="cuts_ms",
        help="Comma-separated candidate hangover cuts in ms defining the cost "
        "bands scanned for the peak at each cell, e.g. '200,400,800,1600' "
        "(sorted + de-duplicated; default: 200,400,800,1600)",
    )
    vad_gap_peak_grid.add_argument(
        "--min-speech-ms",
    ...
    vad_gap_peak_grid.add_argument(
        ...
        default=1,
        dest="top_n",
        help="Name the N steepest cost bands under EACH grid cell instead of just "
        "the single peak (ranked by descending rate, earliest band first on a "
        "tie; only non-empty bands count, so fewer than N may appear) — the "
        "iter-354 single-shot --top-n view, per grid cell, so you can watch how "
    ...
    vad_gap_peak_grid_fmt.add_argument(
        "--csv",
        action="store_true",
        help="Emit a flat <row_axis>,<col_axis>,num_segments,num_gaps,peak_found,"
        "peak_from_ms,peak_to_ms,peak_width_ms,peak_merged_added,"
        "peak_rate_per_100ms CSV table for spreadsheets/plots (mutually "
        "exclusive with --json)",
    )
