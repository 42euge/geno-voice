...
def test_render_sweep_csv_consumer_rederives_json_preference_target_rowmajor_pick():
    # iter-300: iter-297 opened the sweep ROW-MAJOR dict-form cross-surface matrix
    # with the flat SET (the sweep twin of iter-280's grid set-under-row-major test),
    # iter-298 added the CLOSED BAND (twin of iter-275), and iter-299 the OPEN band
    # (twin of iter-273). iter-299 backlog #1 named the next gap directly: the
    # remaining dict forms still lack a sweep ROW-MAJOR cross-surface re-derive — only
    # the speech matrix (iter-290–296) and the scalar/band SCALAR round-trips ever
    # pinned them. The {"prefer": [...]} PREFERENCE is next. The speech version
    # (iter-293) forces tie_break="speech", so it never exercises the DEFAULT branch of
    ...
    #
    # The preference is UNIQUE among the sweep cross-surface forms: its precedence
    # lives at the SORT-KEY layer, not the distance. grid_cell_distance treats a
    # {"prefer": [...]} IDENTICALLY to a flat set (the MIN over its elements —
    # iter-249), and grid_cell_sort_key inserts _preference_rank as a SECONDARY key
    # BEFORE the tie-break key (iter-249). So under the DEFAULT row-major tie-break the
    # key is TWO-level (distance, preference_rank) — NOT the single-level (distance,)
    # key of the band/set/open-band row-major twins (iter-298/297/299), and NOT the
    # three-level (distance, rank, -speech) key of the iter-293 preference-under-speech
    # twin. The preference rank, not row position, breaks a distance tie; row-major
    # only decides rows that ALSO tie on preference rank. That is a DISTINCT mechanism:
    # the band/set/open-band row-major ties resolve PURELY on row position (no key
    # between distance and the stable sort), whereas here the rank OUTRANKS row order,
    # so a LATER row nearer a MORE-preferred element beats an EARLIER row nearer a
    # less-preferred one. The target FORM (grid_cell_distance — which rows tie at the
    # floor) and the tie-break layering (grid_cell_sort_key — how the tied rows ORDER)
    # are independent seams; pinning the band/set/open-band under row-major on the
    # sweep (iter-298/297/299) does NOT pin the preference under row-major.
    #
    # The gap this closes: a regression confined to the DEFAULT row-major path — one
    # that flattened the {"prefer": [...]} dict to a plain set on the sweep JSON path
    # (dropping the _preference_rank secondary key so the row-major tie falls back to
    # pure row position) while a CSV consumer still passed the preference, OR that
    # quietly swapped the row-major fallback for a speech-ordered one on the JSON path
    # only — would diverge the two surfaces yet ship green, because the iter-293
    # preference test forces "speech" (so it never exercises the row-major fallback
    # under a preference) and the scalar/band/set/open-band row-major round-trips never
    # insert a preference rank key for the earliest-tie rule to be OVERRIDDEN by.
    #
    # Fixture: a 4-value threshold sweep with counts 8/4/6/1, target = preference 5>9
    # ({"prefer": [5, 9]}). _sweep_results couples speech to count + index, so the
    # in-preference rows carry DIFFERENT speech and row position vs preference rank
    # genuinely DISAGREE. Preference distance is the MIN over its elements, IDENTICAL
    # to a set:
    #   - 0.30 count 8 → min(|8-5|,|8-9|)=1, rank 1 (nearer 9), speech 4.0
    #   - 0.50 count 4 → min(|4-5|,|4-9|)=1, rank 0 (nearer 5), speech 2.4
    #   - 0.70 count 6 → min(|6-5|,|6-9|)=1, rank 0 (nearer 5), speech 4.2
    #   - 0.90 count 1 → min(|1-5|,|1-9|)=4, rank 0,           speech 0.8
    # THREE rows tie at distance 1 (counts 8, 4, 6). The DEFAULT row-major tie-break
    # would keep the EARLIEST (count 8, 0.30) — but the preference rank OUTRANKS row
    # position, so the two rank-0 rows (counts 4 and 6, nearer the MORE-preferred 5)
    # beat the rank-1 count-8 row (nearer the less-preferred 9) DESPITE it being
    # earliest AND having more speech (4.0s). Among the two rank-0 rows, row-major then
    # keeps the EARLIER one, count 4 (0.50). So the preference+row-major best is the
    # 0.50 row count 4 — the TWO-level key (distance 1, rank 0) selecting it, row-major
    # only deciding between the two rank-0 rows.
    thresholds = [0.3, 0.5, 0.7, 0.9]
    results = _sweep_results([8, 4, 6, 1])
    target = {"prefer": [5, 9]}
    csv_text = gv.render_vad_sweep_csv(thresholds, results, name="rec.wav")
    payload = json.loads(
        # No tie_break kwarg — exercise the DEFAULT row-major path explicitly.
    ...
    # The JSON twin records the preference target (a {"prefer": [...]} object,
    # preserving the listed order) and the DEFAULT tie-break it actually used.
    assert payload["target"] == {"prefer": [5, 9]}
    assert payload["tie_break"] == "row-major"
    # The CSV body still carries no pick columns, tie_break, or preference — it is the
    # bare sweep.
    assert "best" not in csv_text and "distance" not in csv_text
    assert "tie_break" not in csv_text and "prefer" not in csv_text
    # A CSV consumer re-parses the flat table back to sweep rows...
    ...
    # ...and re-runs the SAME picker with the SAME preference target and the DEFAULT
    # (row-major) tie-break. The CSV-derived best equals the JSON `best` (distance key
    # stripped), and the re-derived preference distance matches the one the JSON
    # embedded (1 — one off the nearest preference element 5).
    csv_best = gv.pick_best_grid_cell(rows, target)
    ...
    assert csv_best["threshold"] == 0.5 and csv_best["num_segments"] == 4
    assert payload["best"]["distance"] == 1
    # CONTROL 1: the SAME preference under tie_break="speech" picks a DIFFERENT row —
    # the MOST-speech rank-0 row, count 6 (0.70) at 4.2s, NOT the earliest rank-0 row.
    # So the default pick is genuinely row-major-specific among the rank-0 pair, not a
    # value both tie-breaks share; the default branch is load-bearing for the count-4
    # pick. (Both tie-breaks still honour the rank — neither picks the rank-1 count-8
    # row — so this control isolates the row-major-vs-speech choice WITHIN rank 0.)
    speech_best = gv.pick_best_grid_cell(rows, target, "speech")
    assert speech_best != csv_best
    assert speech_best["threshold"] == 0.7 and speech_best["num_segments"] == 6
    # CONTROL 2: a flat SET of the SAME two elements [5, 9] carries NO precedence —
    # grid_cell_sort_key inserts no preference rank — so under the SAME row-major
    # tie-break the key drops to single-level (distance,) and ALL three dist-1 rows
    # keep pure row position, electing the EARLIEST one, count 8 (0.30). A DIFFERENT
    # pick from the preference best, proving the preference rank is load-bearing:
    # flattening {"prefer": [5,9]} to [5,9] on either surface would change the pick
    # (the rank-0 protection of count 4 vanishes, so the earliest row count 8 — rank 1
    # under the preference — wins on bare row position).
    set_best = gv.pick_best_grid_cell(rows, [5, 9])
    assert set_best != csv_best
    assert set_best["threshold"] == 0.3 and set_best["num_segments"] == 8
    # The top shortlist agrees row-for-row (distance stripped). Ordering is by the
    # TWO-level key: the two rank-0 rows lead (row-major-ordered among themselves,
    # count 4 at 0.50 before count 6 at 0.70), THEN the rank-1 row (count 8), DESPITE
    # count 8 being EARLIEST and having more speech — the preference rank outranks both
    # row position and speech. The dist-4 out-of-preference count-1 row never reaches
    # the top 3.
    csv_top = gv.pick_top_grid_cells(rows, target, 3)
    json_top = [dict(c) for c in payload["top"]]
    for c in json_top:
    ...
    # Head of the shortlist is the single best pick, on both surfaces.
    assert csv_top[0] == csv_best
    # The two rank-0 rows lead (row-major-ordered: count 4 then count 6), THEN the
    # rank-1 count-8 row — NOT the pure row-major order [count 8, count 4, count 6] the
    # set control would give. The preference rank reorders ahead of row position.
    assert [r["num_segments"] for r in csv_top] == [4, 6, 8]
    # All three leading rows scored the preference floor (distance 1 — each one off its
    # nearest element); the preference rank then row position, NOT distance, decided
    # their order. The earliest row (count 8, 0.30) sorts LAST of the three, proving
    # row position is subordinate to the rank.
    assert [c["distance"] for c in payload["top"]] == [1, 1, 1]
    # And the leading rows follow rank-then-row-major order: the two rank-0 rows by
    # threshold (0.50 then 0.70), THEN the rank-1 row (0.30) — the count-8 row that
    # would lead under a bare set, demoted by its worse preference rank.
...
def test_render_sweep_csv_consumer_rederives_json_scaled_target_rowmajor_pick():
    # iter-302: iter-297 opened the sweep ROW-MAJOR dict-form cross-surface matrix
    # with the flat SET (the sweep twin of iter-280's grid set-under-row-major test),
    # iter-298 added the CLOSED BAND (twin of iter-275), iter-299 the OPEN band (twin
    # of iter-273), iter-300 the {"prefer": [...]} PREFERENCE (twin of iter-276), and
    # iter-301 the {"weighted": [...]} ADDITIVE-WEIGHTED set (twin of iter-277).
    # iter-301 backlog #1 named the next gap directly: the remaining dict forms still
    # lack a sweep ROW-MAJOR cross-surface re-derive — only the speech matrix
    # (iter-290–296) and the scalar/band SCALAR round-trips ever pinned them. The
    # {"scaled": [...]} MULTIPLICATIVE set is next. The speech version (iter-295)
    # forces tie_break="speech", so it never exercises the DEFAULT branch of
    # render_vad_sweep_json (the branch taken when no tie_break= kwarg is supplied —
    ...
    #
    # The scaled set is the MULTIPLICATIVE twin of iter-301's weighted set: like the
    # weighted set its precedence lives in the DISTANCE itself, not the sort key.
    # grid_cell_distance MULTIPLIES each element's raw distance by its factor and takes
    # the MIN over those SCALED distances (iter-252), and grid_cell_sort_key inserts NO
    # secondary key for a scaled set (the precedence is already baked into the
    # distance). So under the DEFAULT row-major tie-break the key is the SAME
    # single-level (distance,) shape as the band/set/open-band/weighted twins
    # (iter-298/297/299/301) — but the DISTANCE feeding it is the SCALED one, NOT the
    # raw set distance. The COMPOSITION differs from iter-301 in a way no prior
    # row-major sweep twin exercises: where the weighted set's +penalty taxes even an
    # EXACT HIT (lifting a free 0 above a near miss), the scaled set's *factor leaves an
    # exact hit FREE (0*factor == 0) and only AMPLIFIES an existing NON-zero gap. So the
    # fixture geometry inverts: iter-301 demoted an exact hit below a near-miss floor;
    # here the factor must demote a NEAR-MISS on a high-factor element below the
    # near-miss floor on a low-factor one.
    #
    # The gap this closes: a regression confined to the DEFAULT row-major path — one
    # that flattened the {"scaled": [...]} dict to a plain set on the sweep JSON path
    # (dropping the factor so the high-factor element's near-miss reverts to the same
    # raw distance as the low-factor one and the row-major floor moves) while a CSV
    # consumer still passed the factors, OR that quietly swapped the row-major fallback
    # for a speech-ordered one on the JSON path only — would diverge the two surfaces
    # yet ship green, because the iter-295 scaled test forces "speech" (so it never
    # exercises the row-major fallback under a scaled set) and the
    # scalar/band/set/open-band/preference/weighted row-major round-trips never fold a
    # *factor INTO the distance for the row-major floor to be MOVED by (the weighted
    # twin folds an ADDITIVE penalty, which behaves differently on an exact hit).
    #
    # Fixture: a 4-value threshold sweep with counts 9/4/6/1, target = scaled set
    # 5(*1), 8(*3) ({"scaled": [(5, 1), (8, 3)]}). _sweep_results couples speech to
    # count + index, so the in-floor rows carry DIFFERENT speech and the row-major /
    # speech tie-breaks genuinely DISAGREE. Each element's distance is its raw distance
    # TIMES its factor, the set scoring the MIN over those scaled distances:
    #   - 0.30 count 9 → min(|9-5|*1, |9-8|*3)=min(4,3)=3, speech 4.5
    #   - 0.50 count 4 → min(|4-5|*1, |4-8|*3)=min(1,12)=1, speech 2.4
    #   - 0.70 count 6 → min(|6-5|*1, |6-8|*3)=min(1,6)=1, speech 4.2
    #   - 0.90 count 1 → min(|1-5|*1, |1-8|*3)=min(4,21)=4, speech 0.8
    # The *3 factor on element 8 is load-bearing: count 9 is ONE off element 8 (raw
    # distance 1) but the *3 factor lifts that to 3, so it NO LONGER ties at the floor —
    # the OPPOSITE direction from iter-301's penalty, which taxed an exact hit. TWO rows
    # tie at the scaled floor distance 1 (counts 4 and 6, both one off the *1 element 5).
    # The DEFAULT row-major tie-break keeps the EARLIEST tied row — count 4 (0.50) —
    # DESPITE count 6 (0.70) recovering more speech (4.2s vs 2.4s). So the row-major best
    # is the 0.50 row count 4, and the top-3 is [4, 6, 9] (the two floor rows in sweep
    # order, THEN the scaled count-9 row).
    thresholds = [0.3, 0.5, 0.7, 0.9]
    results = _sweep_results([9, 4, 6, 1])
    target = {"scaled": [(5, 1), (8, 3)]}
    csv_text = gv.render_vad_sweep_csv(thresholds, results, name="rec.wav")
    payload = json.loads(
        # No tie_break kwarg — exercise the DEFAULT row-major path explicitly.
        gv.render_vad_sweep_json(
    ...
    # The JSON twin records the scaled target (a {"scaled": [...]} object — JSON renders
    # the (element, factor) tuples as 2-element lists) and the DEFAULT tie-break it used.
    assert payload["target"] == {"scaled": [[5, 1], [8, 3]]}
    assert payload["tie_break"] == "row-major"
    # The CSV body still carries no pick columns, tie_break, or scaling — it is the bare
    # sweep.
    assert "best" not in csv_text and "distance" not in csv_text
    ...
    # ...and re-runs the SAME picker with the SAME scaled target and the DEFAULT
    # (row-major) tie-break. The CSV-derived best equals the JSON `best` (distance key
    # stripped), and the re-derived scaled distance matches the one the JSON embedded
    # (1 — one off the *1 element 5).
    csv_best = gv.pick_best_grid_cell(rows, target)
    ...
    assert csv_best["threshold"] == 0.5 and csv_best["num_segments"] == 4
    assert payload["best"]["distance"] == 1
    # CONTROL 1: the SAME scaled target under tie_break="speech" picks a DIFFERENT row —
    # the MOST-speech floor row, count 6 (0.70) at 4.2s, NOT the earliest floor row. So
    # the default pick is genuinely row-major-specific among the floor pair, not a value
    # both tie-breaks share; the default branch is load-bearing for the count-4 pick.
    speech_best = gv.pick_best_grid_cell(rows, target, "speech")
    assert speech_best != csv_best
    assert speech_best["threshold"] == 0.7 and speech_best["num_segments"] == 6
    # CONTROL 2: a flat SET of the SAME two elements [5, 8] carries NO factor —
    # grid_cell_distance scores the RAW set MIN — so count 9 is a one-off near miss on
    # element 8 (distance 1) that JOINS the count-4/count-6 floor, and being the EARLIEST
    # of the three tied rows it wins the row-major pick OUTRIGHT. A DIFFERENT pick from
    # the scaled best, proving the *3 factor is load-bearing: flattening
    # {"scaled": [(5,1),(8,3)]} to [5,8] on either surface would change the pick (the
    # factor is what lifts count 9's near-miss to distance 3, demoting it below the
    # count-4/count-6 near-miss floor it would otherwise lead).
    set_best = gv.pick_best_grid_cell(rows, [5, 8])
    assert set_best != csv_best
    assert set_best["threshold"] == 0.3 and set_best["num_segments"] == 9
    # The top shortlist agrees row-for-row (distance stripped). Ordering is by the
    # single-level (scaled distance,) key with row-major among equal distance: the two
    # floor rows lead (sweep-ordered, count 4 at 0.50 before count 6 at 0.70) DESPITE
    # count 6 having more speech, THEN count 9 — its scaled distance (3) sorts it behind
    # the floor pair. The dist-4 row (count 1) never reaches the top 3.
    csv_top = gv.pick_top_grid_cells(rows, target, 3)
    json_top = [dict(c) for c in payload["top"]]
    for c in json_top:
        c.pop("distance")
    ...
    # Head of the shortlist is the single best pick, on both surfaces.
    assert csv_top[0] == csv_best
    # Order [4, 6, 9] — NOT the speech order [6, 4, 9] the iter-295 speech twin shape
    # gives (count 6 leads there as the most-speech floor row), and NOT the bare-set
    # row-major order [9, 4, 6] the flat-set control gives (count 9 leads as the
    # earliest near-miss once the factor is dropped). The *3 factor pushes count 9's
    # distance to 3 so the floor pair outranks it, and row-major then keeps the two
    # floor rows in sweep order.
    assert [r["num_segments"] for r in csv_top] == [4, 6, 9]
    set_top = gv.pick_top_grid_cells(rows, [5, 8], 3)
    assert [r["num_segments"] for r in set_top] == [9, 4, 6]
    # The two leading rows scored the scaled floor (distance 1 — each one off the *1
    # element 5); count 9 trails at distance 3 (its one-off near-miss on element 8
    # tripled by the *3 factor). The scaled distance, then row position among the floor
    # pair, NOT speech, decided the order — the earlier row (count 4, 0.50) leads the
    # higher-speech row (count 6, 0.70).
    assert [c["distance"] for c in payload["top"]] == [1, 1, 3]
    assert [r["threshold"] for r in csv_top] == [0.5, 0.7, 0.3]


def test_render_sweep_csv_consumer_rederives_json_affine_target_rowmajor_pick():
    # iter-303: iter-297 opened the sweep ROW-MAJOR dict-form cross-surface matrix
    # with the flat SET (the sweep twin of iter-280's grid set-under-row-major test),
    # iter-298 added the CLOSED BAND (twin of iter-275), iter-299 the OPEN band (twin
    ...
    # backlog #1 named the LAST gap directly: the sweep still lacks the row-major
    # cross-surface re-derive for the {"affine": [...]} set (iter-287, grid row-major
    # twin iter-279, sweep speech twin iter-296). This lap closes it — the sweep twin
    # of iter-279's grid affine-under-row-major test — COMPLETING the cross-surface
    # ROW-MAJOR matrix over every --target dict form (scalar -> band -> set -> open
    # band -> preference -> weighted -> scaled -> affine), mirroring the grid
    # iter-273-280 set in full.
    #
    # The affine set GENERALISES iter-301's weighted and iter-302's scaled twins: each
    # element's distance is its raw distance SCALED by its factor THEN OFFSET by its
    # penalty (distance*factor + penalty), the set scoring the MIN over those affine
    # distances (iter-287). Like both predecessors its precedence lives in the DISTANCE
    # itself — grid_cell_sort_key inserts NO secondary key for an affine set — so under
    # the DEFAULT row-major tie-break the key is the SAME single-level (distance,) shape
    # as the band/set/open-band/weighted/scaled twins (iter-298/297/299/301/302). What
    # no prior ROW-MAJOR sweep twin exercises is BOTH weight operators biting AT ONCE on
    # the same fixture: the penalty taxes even an EXACT HIT (0*factor + penalty =
    # penalty, the iter-301 weighted behaviour) AND the factor amplifies a non-zero gap
    # (the iter-302 scaled behaviour). A regression confined to the DEFAULT row-major
    # path — one that flattened the {"affine": [...]} dict to a plain set on the sweep
    # JSON path (dropping BOTH weights so the row-major floor moves) while a CSV consumer
    # still passed them, that dropped EITHER weight alone, OR that quietly swapped the
    # row-major fallback for a speech-ordered one on the JSON path only — would diverge
    # the two surfaces yet ship green, because the iter-296 affine test forces "speech"
    # (so it never exercises the row-major fallback under an affine set) and no other
    # row-major sweep twin folds BOTH a *factor AND a +penalty INTO the distance.
    #
    # Fixture: a 4-value threshold sweep with counts 4/9/2/8, target = affine set
    # 5(*1+0), 9(*3+1) ({"affine": [(5, 1, 0), (9, 3, 1)]}). _sweep_results couples
    # speech to count + index, so the in-floor rows carry DIFFERENT speech and the
    # row-major / speech tie-breaks genuinely DISAGREE. Each element's distance is
    # raw*factor + penalty, the set scoring the MIN over those affine distances:
    #   - 0.30 count 4 → min(|4-5|*1+0, |4-9|*3+1)=min(1,16)=1, speech 2.0
    #   - 0.50 count 9 → min(|9-5|*1+0, |9-9|*3+1)=min(4, 1)=1, speech 5.4
    #   - 0.70 count 2 → min(|2-5|*1+0, |2-9|*3+1)=min(3,22)=3, speech 1.4
    #   - 0.90 count 8 → min(|8-5|*1+0, |8-9|*3+1)=min(3, 4)=3, speech 6.4
    # The floor PAIR shows BOTH weight operators biting AT ONCE, in OPPOSITE directions:
    # count 4 is an UNTAXED near-miss on the 0-penalty element 5 (raw 1 → 1*1+0 = 1),
    # while count 9 is an EXACT HIT on element 9 (raw 0) that the +1 penalty taxes UP to
    # 1 (0*3+1 = 1) — so an exact hit and a near-miss are leveled to the SAME floor only
    ...
    # count 4 (0.30) — DESPITE count 9 (0.50) recovering more speech (5.4s vs 2.0s). So
    # the row-major best is the 0.30 row count 4, and the top-3 is [4, 9, 2] (the two
    # floor rows in sweep order, THEN count 2 — its min affine distance 3, |2-5|*1+0,
    # ties count 8's 3 but count 2 is the earlier row).
    thresholds = [0.3, 0.5, 0.7, 0.9]
    results = _sweep_results([4, 9, 2, 8])
    target = {"affine": [(5, 1, 0), (9, 3, 1)]}
    csv_text = gv.render_vad_sweep_csv(thresholds, results, name="rec.wav")
    payload = json.loads(
        # No tie_break kwarg — exercise the DEFAULT row-major path explicitly.
    ...
    # The JSON twin records the affine target (an {"affine": [...]} object — JSON renders
    # the (element, factor, penalty) tuples as 3-element lists) and the DEFAULT tie-break
    # it used.
    assert payload["target"] == {"affine": [[5, 1, 0], [9, 3, 1]]}
    assert payload["tie_break"] == "row-major"
    # The CSV body still carries no pick columns, tie_break, or weights — it is the bare
    ...
    assert csv_best["threshold"] == 0.3 and csv_best["num_segments"] == 4
    assert payload["best"]["distance"] == 1
    # CONTROL 1: the SAME affine target under tie_break="speech" picks a DIFFERENT row —
    # the MOST-speech floor row, count 9 (0.50) at 5.4s, NOT the earliest floor row. So
    # the default pick is genuinely row-major-specific among the floor pair, not a value
    # both tie-breaks share; the default branch is load-bearing for the count-4 pick.
    speech_best = gv.pick_best_grid_cell(rows, target, "speech")
    assert speech_best != csv_best
    assert speech_best["threshold"] == 0.5 and speech_best["num_segments"] == 9
    # CONTROL 2: a flat SET of the SAME two elements [5, 9] carries NEITHER weight —
    # grid_cell_distance scores the RAW set MIN — so count 9 becomes a FREE exact hit
    # (distance 0) and wins the row-major pick OUTRIGHT (the only dist-0 row). A DIFFERENT
    # pick from the affine best, proving the weights are load-bearing: flattening
    # {"affine": [(5,1,0),(9,3,1)]} to [5,9] on either surface would change the pick (the
    # +1 penalty is what lifts count 9's exact hit to distance 1, leveling it with count
    # 4's near-miss instead of letting it lead alone).
    set_best = gv.pick_best_grid_cell(rows, [5, 9])
    assert set_best != csv_best
    assert set_best["threshold"] == 0.5 and set_best["num_segments"] == 9
    # CONTROL 3: dropping ONLY the penalty (the {"scaled": [(5,1),(9,3)]} twin of
    # iter-302) makes count 9 a FREE exact hit again (0*3 = 0), so it leads the row-major
    # shortlist OUTRIGHT — the scaled top is [9, 4, 2], NOT the affine [4, 9, 2]. A
    # DIFFERENT shortlist head, proving the +1 penalty is independently load-bearing —
    # dropping it on either surface would change the pick (it is the ONLY thing leveling
    # count 9's exact hit up to the count-4 floor).
    scaled_top = gv.pick_top_grid_cells(rows, {"scaled": [(5, 1), (9, 3)]}, 3)
    assert [r["num_segments"] for r in scaled_top] == [9, 4, 2]
    # CONTROL 4: dropping ONLY the factor (the {"weighted": [(5,0),(9,1)]} twin of
    # iter-301) lowers count 8's near-miss on element 9 from raw 1 → 1*3+1=4 (affine)
    # down to 1+1=2 (weighted), so count 8 overtakes count 2 for the 3rd top slot. The
    # weighted top is [4, 9, 8], NOT the affine [4, 9, 2]. A DIFFERENT shortlist tail,
    # proving the *3 factor is independently load-bearing — dropping it on either surface
    # would change the pick. CONTROLS 3 and 4 together pin that BOTH affine weights
    # matter at once, the property no weighted-only (iter-301) or scaled-only (iter-302)
    # row-major sweep twin can establish.
    weighted_top = gv.pick_top_grid_cells(rows, {"weighted": [(5, 0), (9, 1)]}, 3)
    assert [r["num_segments"] for r in weighted_top] == [4, 9, 8]
    # The top shortlist agrees row-for-row (distance stripped). Ordering is by the
    # single-level (affine distance,) key with row-major among equal distance: the two
    # floor rows lead (sweep-ordered, count 4 at 0.30 before count 9 at 0.50) DESPITE
    # count 9 having more speech, THEN count 2 — its min affine distance (3) ties count
    # 8's 3 but count 2 is the earlier row.
    csv_top = gv.pick_top_grid_cells(rows, target, 3)
    json_top = [dict(c) for c in payload["top"]]
    for c in json_top:
        c.pop("distance")
    ...
    # Head of the shortlist is the single best pick, on both surfaces.
    assert csv_top[0] == csv_best
    # Order [4, 9, 2] — UNIQUE to the affine target under row-major. The flat-set control
    # gives a count-9-led order (free exact hit), the scaled-only control [9, 4, 2]
    # (penalty dropped, count 9 free), the weighted-only control [4, 9, 8] (factor
    # dropped, count 8 recovers the tail). Only when BOTH weights bite under row-major
    # does count 4 lead (earliest floor) and count 2 (earlier of the two dist-3 rows)
    # take the tail over count 8.
    assert [r["num_segments"] for r in csv_top] == [4, 9, 2]
    set_top = gv.pick_top_grid_cells(rows, [5, 9], 3)
    assert [r["num_segments"] for r in set_top] == [9, 4, 8]
    # The two leading rows scored the affine floor (distance 1 — count 4 a 0-penalty
    # near-miss on element 5, count 9 an exact hit on element 9 taxed up by +1); count 2
    # trails at distance 3 (its min affine distance: |2-5|*1+0=3 beats |2-9|*3+1=22). The
    # affine distance, then row position among the floor pair, NOT speech, decided the
    # order — the earlier row (count 4, 0.30) leads the higher-speech row (count 9, 0.50).
    assert [c["distance"] for c in payload["top"]] == [1, 1, 3]
    assert [r["threshold"] for r in csv_top] == [0.3, 0.5, 0.7]
