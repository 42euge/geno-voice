...
def test_missing_transport_dependency_has_install_hint(monkeypatch) -> None:
    monkeypatch.setattr(
        "geno_voice.endpoint.host.importlib.util.find_spec", lambda name: None
    )

    with pytest.raises(RuntimeError, match=r"grpcio.*geno-voice\[endpoint\]"):
...
def test_breeze_license_warning_is_printed_at_launch() -> None:
    async def scenario() -> None:
        ...
        await run_endpoint_async(
            EndpointConfig(protocol="rtp", model="breeze-tts-2"),
