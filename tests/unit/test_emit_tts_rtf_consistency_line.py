"""Tests for iter-141 — _emit_tts_rtf_consistency_line.

Sixth instance of the diversity-check pattern. Third applied
to a CONTINUOUS metric (after iter-128 sentence-length and
iter-140 stt-rtf) — buckets the per-turn ``tts_rtf`` via
``_tts_rtf_bucket`` before scanning. Detects 5+ consecutive turns
that landed in the "slow" or "very_slow" bucket, suggesting the
TTS engine/voice is too heavy for the host hardware (synth, not
playback, is the bottleneck).
"""
...
def test_bucket_zero_returns_empty():
    """0 RTF = no audio produced / unmeasured turn → empty bucket
    (filtered by the consumer)."""
    assert _tts_rtf_bucket(0) == ""
...
def test_bucket_negative_returns_empty():
    ...
    assert _tts_rtf_bucket(-1.0) == ""


def test_bucket_realtime_boundary():
    """< 1.0 → realtime (the desired state). 0.999 is the upper
    edge."""
    assert _tts_rtf_bucket(0.1) == "realtime"
    assert _tts_rtf_bucket(0.999) == "realtime"


def test_bucket_slow_boundary():
    """1.0-2.0 inclusive → slow."""
    assert _tts_rtf_bucket(1.0) == "slow"
    assert _tts_rtf_bucket(2.0) == "slow"


def test_bucket_very_slow_boundary():
    """> 2.0 → very_slow."""
    assert _tts_rtf_bucket(2.001) == "very_slow"
    assert _tts_rtf_bucket(10.0) == "very_slow"


def test_bucket_handles_floats():
    """tts_rtf is a float — bucket must handle fine-grained
    values around the boundaries."""
    assert _tts_rtf_bucket(0.9999) == "realtime"
    assert _tts_rtf_bucket(1.0001) == "slow"
    assert _tts_rtf_bucket(2.0) == "slow"
    assert _tts_rtf_bucket(2.0001) == "very_slow"
...
def test_empty_list_emits_nothing():
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [])
    assert lines == []


def test_all_zero_rtf_emit_nothing():
    """All turns produced no audio (0 RTF) → no warning."""
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [0.0] * 10)
    assert lines == []
...
def test_long_realtime_run_does_not_fire():
    """A 10-turn run of fast (sub-realtime) TTS is the desired
    state — never flagged."""
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [0.2] * 10)
    assert lines == []
...
def test_five_slow_in_a_row_fires():
    """Default threshold = 5."""
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [1.5] * 5)
    assert len(lines) == 1
    assert "TTS speed" in lines[0]
    assert "5 consecutive" in lines[0]
    assert "'slow'" in lines[0]
    assert "lighter voice or pre-rendered fillers" in lines[0]
    assert "iter-050" in lines[0]


def test_six_very_slow_in_a_row_fires():
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [3.0] * 6)
    assert len(lines) == 1
...
def test_below_threshold_does_not_fire():
    ...
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [1.5] * 4)
    assert lines == []
...
def test_threshold_3_catches_smaller_pattern():
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [3.0] * 3, threshold=3)
    assert "3 consecutive" in lines[0]


def test_threshold_10_suppresses_default_5_run():
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [3.0] * 5, threshold=10)
    assert lines == []
...
def test_longer_very_slow_run_beats_shorter_slow_run():
    ...
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [1.5] * 4 + [3.0] * 7)
...
def test_warning_includes_iter_050_attribution():
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [1.5] * 5)
    assert "iter-050" in lines[0]


# ---- Pattern parity with iter-114/115/120/126/128/140 ---------------
...
def test_iter_116_helper_handles_large_input():
    ...
    emit, lines = _capture()
    _emit_tts_rtf_consistency_line(emit, [1.5] * 1000)
